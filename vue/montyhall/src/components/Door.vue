<template>
    <div class="door-area"> <!--estrutura da porta-->
        <div class="door-frame" :class="{ selected: selected && !open }">
            <Gift v-if="open && hasGift" /> <!--se os dois forem verdadeiros apresenta o presente-->
        </div>
        <div class="door" :class="{ open }" 
            @click="selected = !selected"> <!--a porta em si //// no clique inverte-se o valor-->
            <div class="number" :class="{ selected }">{{ number }}</div>
            <div class="knob" :class="{ selected }"
                @click.stop="open = true"></div> <!--maçaneta-->
        </div>
    </div>
</template>
